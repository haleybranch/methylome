...
################### NORTH ############################################
#### WET ######
mod_nw <- lmer(FT~Rep + Site + PrePeak*Treatment_history + (1|S0_ID), data=dat_north_wet)
#log 
dat_north_wet$FT_log <- log(dat_north_wet$FT)
mod_nw_log <- lmer(FT_log~Rep + Site + PrePeak*Treatment_history + (1|S0_ID), data=dat_north_wet)
...
effsize_nw <- omega_squared(mod_nw_log)
effsize_nw
#write.csv(effsize_nw, "Results/FT_nw_effectsize.csv")
...
plot_nw <- ggplot(Res_fl_nw, aes(Treatment_history, y=exp(visregRes)))+
...
## 
mod_nd <- lmer(FT~Rep + Site + PrePeak*Treatment_history + (1|S0_ID), data=dat_north_dry)

#log 
dat_north_dry$FT_log <- log(dat_north_dry$FT)
mod_nd_log <- lmer(FT_log~Rep + Site + PrePeak*Treatment_history + (1|S0_ID), data=dat_north_dry)
...
plot_nd <- ggplot(Res_fl_nd, aes(Treatment_history, y=exp(visregRes)))+
  ...
  facet_wrap(.~ PrePeak) +
  ylab("Flowering Time")+
  ylim(45,120)+  scale_color_manual(values= c("Dry"="#FFA100", "Wet"="skyblue3"))+
  scale_fill_manual(values= c("Dry"="#FFA100", "Wet"="skyblue3"))+
  theme_classic()
...
mod_sw <- lmer(FT~Rep + Site + PrePeak*Treatment_history + (1|S0_ID), data=dat_south_wet)
...
adjust.p.df
#no difference
#write.csv(adjust.p.df, "Results/FT_sw_planned.csv")
...
###### DRY ############
## 
mod_sd <- lmer(FT~Rep + Site + PrePeak*Treatment_history + (1|S0_ID), data=dat_south_dry)
