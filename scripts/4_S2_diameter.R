...
mod_nw <- lmer(Diameter ~ Rep + Site + PrePeak*Treatment_history + (1|S0_ID), data=dat_north_wet)
...
anova_nw
#treatment history = 0.007
#write.csv(anova_nw, "Results/diameter_nw_anova.csv")
...
effsize_nw
#TRTHIS = 0.02
#write.csv(effsize_nw, "Results/diameter_nw_effectsize.csv")
...
adjust.p.df
#write.csv(adjust.p.df, "Results/diameter_nw_planned.csv")
#pre DW-WW = 0.03 --> 
...
mod_nd <- lmer(Diameter ~ Rep + Site + PrePeak*Treatment_history + (1|S0_ID), data=dat_north_dry)
...
anova_nd
#nothing
#write.csv(anova_nd, "Results/diameter_nd_anova.csv")
...
effsize_nd
#TRTHIS = 0.02
#write.csv(effsize_nd, "Results/diameter_nd_effectsize.csv")
###
...
adjust.p.df
#no differences
#write.csv(adjust.p.df, "Results/diameter_nd_planned.csv")
...
plot_nd <- ggplot(Res_sla_nd, aes(Treatment_history, y=visregRes))+
  ...
  facet_wrap(.~ PrePeak) +
  ylim(-0.5,8)+
  ylab("Stem Diameter (mm)")+  scale_color_manual(values= c("Dry"="#FFA100", "Wet"="skyblue3"))+
  scale_fill_manual(values= c("Dry"="#FFA100", "Wet"="skyblue3"))+
  theme_classic()
...
mod_sw <- lmer(Diameter ~ Rep + Site + PrePeak*Treatment_history + (1|S0_ID), data=dat_south_wet)
...
anova_sw
#nothing
#write.csv(anova_sw, "Results/diameter_sw_anova.csv")
...
effsize_sw
#write.csv(effsize_sw, "Results/diameter_sw_effectsize.csv")
...
adjust.p.df
#nothing
#write.csv(adjust.p.df, "Results/diameter_sw_planned.csv")
...
mod_sd <- lmer(Diameter ~ Rep + Site + PrePeak*Treatment_history + (1|S0_ID), data=dat_south_dry)
...
anova_sd
#treatment history = 0.03
#write.csv(anova_sd, "Results/diameter_sd_anova.csv")
...
effsize_sd
#TRTHIS = 0.02
#write.csv(effsize_sd, "Results/diameter_sd_effectsize.csv")
...
adjust.p.df<- adjust.p %>%
  ...
  inner_join(summ.contrast)
adjust.p.df
#write.csv(adjust.p.df, "Results/diameter_sd_planned.csv")
#peak DD-WW = 0.03 -->
#peak DD-WD = 0.03 --> 
