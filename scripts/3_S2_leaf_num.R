...
dat2 <- dat2 %>% filter(!is.na(nodes))

#################### leaves ##################################################
dat2$nodes<- as.numeric(dat2$nodes)
dat2 <- dat2 %>% filter(!is.na(nodes))
## check datasheet for wet 2159, 2399, 2456 
dat2 <- filter(dat2, ID != "2159" | Treatment != "Wet")
dat2$leaves <- (dat2$nodes * 2)
...
mod_nw <- lmer(leaves ~ Rep + Site + PrePeak*Treatment_history + (1|S0_ID), data=dat_north_wet)
...
anova_nw
#treatment history = 0.01
#write.csv(anova_nw, "Results/leaves_nw_anova.csv")
...
effsize_nw
#TRTHIS = 0.02
#write.csv(effsize_nw, "Results/leaves_nw_effectsize.csv")
...
adjust.p.df
#write.csv(adjust.p.df, "Results/leaves_nw_planned.csv")
#nothing
...
##### north dry ######
mod_nd <- lmer(leaves ~ Rep + Site + PrePeak*Treatment_history + (1|S0_ID), data=dat_north_dry)
...
anova_nd
#nothing
#write.csv(anova_nd, "Results/leaves_nd_anova.csv")
...
effsize_nd
#prepeak = 0.02
#write.csv(effsize_nd, "Results/leaves_nd_effectsize.csv")
...
adjust.p.df<- adjust.p %>%
  ...
  inner_join(summ.contrast)
adjust.p.df
#write.csv(adjust.p.df, "Results/leaves_nd_planned.csv")
#nothing
...
mod_sw <- lmer(leaves ~ Rep + Site + PrePeak*Treatment_history + (1|S0_ID), data=dat_south_wet)
...
anova_sw
#treatment history = 0.06
#write.csv(anova_sw, "Results/leaves_sw_anova.csv")
...
effsize_sw
#TRTHIS = 0.01
#write.csv(effsize_sw, "Results/leaves_sw_effectsize.csv")
...
adjust.p.df<- adjust.p %>%
  ...
  inner_join(summ.contrast)
adjust.p.df
#write.csv(adjust.p.df, "Results/leaves_sw_planned.csv")
#nothing
...
mod_sd <- lmer(leaves ~ Rep + Site + PrePeak*Treatment_history + (1|S0_ID), data=dat_south_dry)
...
anova_sd
#treatment history = 0.02
#write.csv(anova_sd, "Results/leaves_sd_anova.csv")
...
effsize_sd
#TRTHIS = 0.02
#write.csv(effsize_sd, "Results/leaves_sd_effectsize.csv")
...
adjust.p.df<- adjust.p %>%
  ...
  inner_join(summ.contrast)
adjust.p.df
#write.csv(adjust.p.df, "Results/leaves_sd_planned.csv")
#nothing
...
plot_sd <- ggplot(Res_sla_sd, aes(Treatment_history, y=visregRes))+
  ...
  facet_wrap(.~ PrePeak) +
  ylab("Leaf Number")+
  ylim(2,45)+  scale_color_manual(values= c("Dry"="#FFA100", "Wet"="skyblue3"))+
  scale_fill_manual(values= c("Dry"="#FFA100", "Wet"="skyblue3"))+
  theme_classic()
