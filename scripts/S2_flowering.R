...
rep1 <- read.csv("Data/S2_rep1_phenology.csv")
rep1$Rep <- "1"
rep2 <- read.csv("Data/S2_rep2_phenology.csv")
rep2$Rep <- "2"
dat <- rbind(rep1,rep2)
...
dat2 <- dat2 %>% filter(!is.na(FT))
dat2$FD <- as.numeric(dat2$FD)

#remove any plants that took longer than 120 days from sow date to flowering
#this is because it is ecologically not relevant as the growing season is maximum 4 months
...
################### NORTH ############################################
#### WET ######
mod_nw <- lmer(FT~Rep + PrePeak*Treatment_history + (1|Site/S0_ID), data=dat_north_wet)
#log 
dat_north_wet$FT_log <- log(dat_north_wet$FT)
mod_nw_log <- lmer(FT_log~Rep + PrePeak*Treatment_history + (1|Site/S0_ID), data=dat_north_wet)


#check for model violations
plot(mod_nw_log, type=c("p","smooth"), col.line=1) #fitted vs residuals
...
lattice::qqmath(mod_nw_log) # qqplot


hist(resid(mod_nw), breaks = 40) #histogram
hist(resid(mod_nw_log), breaks = 40) #histogram

plot(mod_nw, rstudent(.) ~ hatvalues(.)) #residuals vs leverage


#anova
FT_anova_nw <- anova(mod_nw_log)
FT_anova_nw
#no sign. 
#write.csv(FT_anova_nw, "Results/FT_nw_anova.csv")


###
emm1 = emmeans(mod_nw_log, specs = ~ PrePeak*Treatment_history)
emm1
...
adjust.p.df
#nothing
#write.csv(adjust.p.df, "Results/FT_nw_planned.csv")


vis_fl_nw<-visreg(mod_nw_log, xvar="Treatment_history", by="PrePeak") 
Res_fl_nw<-vis_fl_nw$res # Extract residuals

Res_fl_nw$PrePeak <- factor(Res_fl_nw$PrePeak, level = c('Pre', 'Peak'))
Res_fl_nw$Treatment_history <- factor(Res_fl_nw$Treatment_history, level = c('DD', 'DW', "WD", "WW"), labels = c("DDW", "DWW", "WDW", "WWW"))
...
plot_nw <- ggplot(Res_fl_nw, aes(Treatment_history, y=visregRes))+
 ...
 # facet_wrap(.~ PrePeak) +
  scale_y_continuous(name="Log Flowering Time")+
...
## 
mod_nd <- lmer(FT~Rep + PrePeak*Treatment_history + (1|Site/S0_ID), data=dat_north_dry)

#log 
dat_north_dry$FT_log <- log(dat_north_dry$FT)
mod_nd_log <- lmer(FT_log~Rep + PrePeak*Treatment_history + (1|Site/S0_ID), data=dat_north_dry)



#check for model violations
plot(mod_nd_log, type=c("p","smooth"), col.line=1) #fitted vs residuals
...
hist(resid(mod_nd), breaks = 40) #histogram
hist(resid(mod_nd_log), breaks = 40) #histogram

plot(mod_nd_log, rstudent(.) ~ hatvalues(.)) #residuals vs leverage

#anova
FT_aov_nd <- anova(mod_nd_log) 
FT_aov_nd
# prepeak p = 0.08
#write.csv(FT_aov_nd, "Results/FT_nd_anova.csv")
...
effsize_nd
#prepeak = 0.11
#write.csv(effsize_nd, "Results/FT_nd_effectsize.csv")
...
adjust.p.df
#nothing
#write.csv(adjust.p.df, "Results/FT_nd_planned.csv")
...
plot_nd <- ggplot(Res_fl_nd, aes(PrePeak, y=visregRes))+
  geom_violin(aes(), fill ="#FFA100", position = position_dodge(width = 1))+
  stat_summary(aes(), fun=mean, colour="black",position = position_dodge(width = 1))+
  stat_summary(aes(),fun.data=mean_se, geom="errorbar", colour="black", width=0.5,position = position_dodge(width = 1))+
 # xlab("Treatment History") +
...
################### SOUTH ############################################
#### WET ######
#dat_south_wet$FT <- log(dat_south_wet$FT)
mod_sw <- lmer(FT~Rep + PrePeak*Treatment_history + (1|Site/S0_ID), data=dat_south_wet)

#log 
dat_south_wet$FT_log <- log(dat_south_wet$FT)
mod_sw_log <- lmer(FT_log~Rep + PrePeak*Treatment_history + (1|Site/S0_ID), data=dat_south_wet)
...
hist(resid(mod_sw), breaks = 40) #histogram

plot(mod_sw, rstudent(.) ~ hatvalues(.)) #residuals vs leverage

#anova
FT_aov_sw <- anova(mod_sw) 
FT_aov_sw
# trt history = 0.02
#write.csv(FT_aov_sw, "Results/FT_sw_anova.csv")
...
effsize_sw
#trt hist = 0.02
#write.csv(effsize_sw, "Results/FT_sw_effectsize.csv")
...
Res_fl_sw$Treatment_history <- factor(Res_fl_sw$Treatment_history, level = c('DD', 'DW', "WD", "WW"), labels = c("DDW", "DWW", "WDW", "WWW"))
...
plot_sw <- ggplot(Res_fl_sw, aes(Treatment_history, y=visregRes))+
  ...
  xlab("Treatment History") +
  #facet_wrap(.~ PrePeak) +
  scale_y_continuous(name="Flowering Time")+
  scale_color_manual(values= c("Dry"="#FFA100", "Wet"="skyblue3"))+
  scale_fill_manual(values= c("Dry"="#FFA100", "Wet"="skyblue3"))+
  theme_classic()
...
###### DRY ############
## 
mod_sd <- lmer(FT~Rep + PrePeak*Treatment_history + (1|Site/S0_ID), data=dat_south_dry)

dat_south_dry$FT_log <- log(dat_south_dry$FT)
mod_sd_log <- lmer(FT_log~Rep + PrePeak*Treatment_history + (1|Site/S0_ID), data=dat_south_dry)
...
hist(resid(mod_sd), breaks=40) #histogram
...
#prepeak 0.07
#trt hist = 0.09
#write.csv(anova_sd, "Results/FT_sd_anova.csv")
...
#prepeak = 0.13
#trt hist < 0.01
#write.csv(effsize_sd, "Results/FT_sd_effectsize.csv")
...
adjust.p.df<- adjust.p %>%
  ...
  inner_join(summ.contrast)
adjust.p.df
#write.csv(adjust.p.df, "Results/FT_sd_planned.csv")
#nothing
